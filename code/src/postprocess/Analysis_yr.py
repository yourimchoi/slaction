...
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix, classification_report, roc_auc_score, cohen_kappa_score, average_precision_score
...
for center in centers:
    ...
    output_dir = f'../analysis/{exp_number}/{center}'
    os.makedirs(output_dir, exist_ok=True)

    # Load the CSV file
    file_path = f'../result/{exp_number}/{center}_test_results.csv'
    ...
    f1 = f1_score(y_true, y_pred, average='macro')  # Calculate macro-F1 score
    f1_w = f1_score(y_true, y_pred, average='weighted')  # Calculate macro-F1 score
    ...
    # Calculate AUROC and AUPRC for each class in the original 4-class classification
    auroc_4_class = roc_auc_score(pd.get_dummies(y_true), pd.get_dummies(y_pred), average='weighted')
    auprc_4_class = average_precision_score(pd.get_dummies(y_true), pd.get_dummies(y_pred), average='macro')
    ...
    f1_binary = f1_score(y_true_binary, y_pred_binary, average='macro')  # Calculate macro-F1 score
    f1_binary_w = f1_score(y_true_binary, y_pred_binary, average='weighted')  # Calculate macro-F1 score
    ...
    # Calculate AUROC and AUPRC for binary RA vs Non-RA classification
    auroc_binary = roc_auc_score(y_true_binary, y_pred_binary, average='weighted')
    auprc_binary = average_precision_score(y_true_binary, y_pred_binary, average='macro')
    ...
    f1_supine = f1_score(y_true_supine, y_pred_supine, average='macro')  # Calculate macro-F1 score
    f1_supine_w = f1_score(y_true_supine, y_pred_supine, average='weighted')  # Calculate macro-F1 score
    ...
    # Calculate AUROC and AUPRC for binary Supine vs Non-Supine classification
    auroc_supine = roc_auc_score(y_true_supine, y_pred_supine, average='weighted')
    auprc_supine = average_precision_score(y_true_supine, y_pred_supine, average='macro')
    ...
    metrics_df = pd.DataFrame({
        "Metric": ["Accuracy", "Precision", "Recall", "Macro-F1 Score", "Weighted-F1 Score", "AUROC", "AUPRC", "Kappa"],  # Add Kappa to metrics list
        "Original 4-Class": [original_metrics["Accuracy"], original_metrics["Precision"], 
                           original_metrics["Recall"], original_metrics["F1 Score"], original_metrics["F1 W Score"], 
                           original_metrics["AUROC"], original_metrics["AUPRC"], original_metrics["Kappa"]],
        "Binary RA vs Non-RA": [binary_metrics["Accuracy"], binary_metrics["Precision"], 
                               binary_metrics["Recall"], binary_metrics["F1 Score"], binary_metrics["F1 W Score"], 
                               binary_metrics["AUROC"], binary_metrics["AUPRC"], binary_metrics["Kappa"]],
        "Binary Supine vs Non-Supine": [supine_metrics["Accuracy"], supine_metrics["Precision"], 
                                       supine_metrics["Recall"], supine_metrics["F1 Score"], supine_metrics["F1 W Score"], 
                                       supine_metrics["AUROC"], supine_metrics["AUPRC"], supine_metrics["Kappa"]]
    })
...
plt.tight_layout()
plt.savefig(f'../analysis/{exp_number}/overall_metrics_comparison_bar_chart.png')
plt.close()

print(f"Overall metrics comparison bar chart has been successfully saved to ../analysis/{exp_number}.")