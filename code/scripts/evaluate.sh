...
# Function to run the testing script
run_testing() {
    local gpu=$1
    local yaml_index=$2
    local num_workers=$3
    ...
    CUDA_VISIBLE_DEVICES=$gpu python ../src/core/Evaluate.py --config $yaml_path --num_workers $num_workers
    if [ $? -ne 0 ]; then
...
main() {
    ...
    if ! run_testing $gpu $yaml_index $num_workers; then
        echo "Testing failed or was interrupted"
        exit 1
    fi
}
